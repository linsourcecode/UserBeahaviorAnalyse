...
object RecordInfos {
  ...
  def main(args: Array[String]): Unit = {
   ...
   // properties.setProperty("auto.commit.interval.ms", "2000");
    //修改分区增加两个分区
    val inputStream = env.addSource( new FlinkKafkaConsumer011[String]("records", new SimpleStringSchema(), properties) )
    ...
    val chPassword = ""
    //加大批量
    val batchSize = 220 // 设置batch size，批量写入clickhouse，提高性能
